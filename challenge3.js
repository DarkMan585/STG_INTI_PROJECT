...
describe("challenge3 suite", function(){
    ...
    it("Loops through poplar section and print link and text", async function(){
        var popular_array = await driver.findElements(webdriver.By.xpath("//*[@id=\"tabTrending\"]/div[1]/div[2]//a"));
        console.log(popular_array.length);
        for(var i=0; i<popular_array.length; i++){
            var print1 = await popular_array[i].getText();
            var print2 = await popular_array[i].getAttribute("href");
